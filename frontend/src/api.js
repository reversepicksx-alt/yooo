...
export async function getIntelSheet(email, token, sport = 'soccer') {
  return apiCall(`/api/intel/sheet?email=${encodeURIComponent(email)}&token=${encodeURIComponent(token)}&sport=${sport}`);
}
