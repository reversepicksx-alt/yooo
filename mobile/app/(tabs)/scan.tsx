...
export default function ScanScreen() {
  ...
  const selectSport = (next: Sport) => {
    ...
    setPickSaved(false);
    // A sport switch starts a fresh manual lookup. Do not leave a player or
    // club context from another sport attached to the new search boundary.
    setPlayerQuery('');
  ...
  const handleSoccerPlayerSelect = async (p: FuzzyPlayerResult | UniversalPlayerResult) => {
    ...
    if (p.playerId) {
      ...
      try {
        ...
        // Search results are identity hints, not current-club evidence.
        // Never promote a cached club after authoritative context verification
        // fails: a transfer can otherwise create a wrong matchup.
    ...
    try {
      ...
      await savePick(session.email, session.token, {
        ...
        confidenceInterval: prediction.confidenceInterval,
         propHistoricalRate: (prediction as any).propHistoricalRate,
         propHistoricalN: (prediction as any).propHistoricalN,
         lineDeviationHitRate: (prediction as any).lineDeviationHitRate,
         lineDeviationHitRateN: (prediction as any).lineDeviationHitRateN,
        // Keep the sport-specific analysis snapshot on the saved pick. MLB
        // and NFL have dedicated analysis endpoints and must not depend on a
        // rotated prediction cache to reconstruct their history.
        gameLogs: (prediction as any).gameLogs || undefined,
        playerGameLogs: (prediction as any).playerGameLogs || undefined,
        recentValues: (prediction as any).recentValues || undefined,
        hitRates: (prediction as any).hitRates || undefined,
        historyGameCount: (prediction as any).historyGameCount || undefined,
        historySeasons: (prediction as any).historySeasons || undefined,
        historyRange: (prediction as any).historyRange || undefined,
        tacticalBreakdown: prediction.tacticalBreakdown || undefined,
        tacticalAlerts: (prediction as any).tacticalAlerts || undefined,
        evidenceQuality: (prediction as any).evidenceQuality || undefined,
        safetyRating: (prediction as any).safetyRating || undefined,
        lineDeviationBand: (prediction as any).lineDeviationBand || undefined,
        lineDeviationPct: (prediction as any).lineDeviationPct || undefined,
        lineDeviationHitRate: (prediction as any).lineDeviationHitRate || undefined,
        lineDeviationHitRateN: (prediction as any).lineDeviationHitRateN || undefined,
        gameTotalUsed: (prediction as any).gameTotalUsed || undefined,
        gameTotalSource: (prediction as any).gameTotalSource || undefined,
        gameTotal: (prediction as any).gameTotal || undefined,
        pitcherName: (prediction as any).pitcherName || undefined,
        pitcherHandedness: (prediction as any).pitcherHandedness || undefined,
        batterHandedness: (prediction as any).batterHandedness || undefined,
        pitcherEra: (prediction as any).pitcherEra || undefined,
        lineupSpot: (prediction as any).lineupSpot || undefined,
        oppRankPercentile: (prediction as any).oppRankPercentile || undefined,
        restDays: (prediction as any).restDays || undefined,
        ...
        {phase === 'result' && prediction && (
          <>
            ...
            <View style={styles.analysisCard}>
              ...
              {analysisTab === 'matchup' && (() => {
                const landingBands = predictionLandingBands;
              ...
              {/* Line vs Season Average + Edge Explanation — tactical verdict replaces
               ...
               {prediction.priorMean != null && prediction.line != null && (() => {
                ...
                const hitRate = prediction.lineDeviationHitRate;
                 ...
                 const calibrationWarming = prediction.lineDeviationStatus === 'unavailable';
