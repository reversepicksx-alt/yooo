...
**How to apply:** Keep one global owner-only recognizer in the authenticated shell, guard starts with refs/timers, treat final natural sentences as questions when global mode is enabled, reserve the visible control for permission recovery rather than normal stop/start interaction, provide a tap-to-unlock spoken welcome on web, and keep browser response latency bounded independently of the general prediction timeout.