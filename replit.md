...
- The AI architecture exclusively uses Google Gemini (Gemini 2.5 Pro for tactical chat, Gemini 2.5 Flash for predictions, scan OCR, and position resolution). Gemini's JSON mode is preferred for guaranteeing parseable output.
...
- AI-driven press intensity scores should override heuristic methods when available, providing more accurate input for bayesian projections.
...
- **Home CDM Deep-Block Boost**: Symmetric complement to the CDM Inversion Layer for the dominant HOME team scenario. When a home team's expected possession exceeds 60% AND the opponent's expected possession is below 40% (deep-block / parking-the-bus), the CDM/DM/DLP becomes the ball-recycling hub: every possession sequence funnels back through the base midfielder because the opponent's block prevents forward progression. The boost scales with both opponent block depth (0→1 as opp poss drops 40%→22%) and home team dominance (0→1 as team poss rises 60%→72%), capped at +15%. For the canonical miss that triggered this fix (Tchouaméni vs Oviedo, home, La Liga: Real Madrid 65% expected poss vs Oviedo 31%) the engine now applies ×1.081 — boosting a ~68 pass/game prior to ~73.5 = OVER 67.5 (correct). Surfaces in the response under `bayesianMetrics.homeCdmDeepBlock`. The AI prompt also received an explicit rule: never penalise CDM pass counts in dead-rubber/low-stakes matches where the dominant home team is still retaining possession — the passes still happen through the CDM, just more slowly and circularly.
- **AI-Driven Press Intensity**: The AI engine fetches an AI-rated press intensity score (0.0-1.0) for opponents using web search and knowledge, overriding heuristic methods for more accurate projections. This score is cached to optimize performance.
