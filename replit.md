...
- **Pick Card Swipe-to-Delete**: The trash-can icon was removed from each pick card; users now swipe a card to the right to reveal a red `DELETE` action (left-side reveal via `Swipeable` from `react-native-gesture-handler`). On full open, the row snaps closed and the existing Cancel/Delete confirm Alert is shown — so accidental swipes are recoverable. Medium haptic fires on open. The `trackBar` (under the OVER/UNDER pill) was thickened from 2px to 5px with rounded ends to give a chunkier, iOS-like "block" look.
