...
- **Pick Card Swipe-to-Delete**: The trash-can icon was removed from each pick card; users now swipe a card to the right to reveal a persistent red `DELETE` button — iOS Mail / Messages pattern. Built on `ReanimatedSwipeable` from `react-native-gesture-handler/ReanimatedSwipeable` (modern Gesture API, reliable iOS arbitration with the parent FlatList scroll). Tuning: `friction={1.5}`, `leftThreshold={40}`, `dragOffsetFromLeftEdge={6}` for an easy/responsive swipe. The revealed action is a `TouchableOpacity` — tap fires the existing Cancel/Delete confirm Alert. We deliberately do NOT auto-fire delete on `onSwipeableOpen` (that would dismiss the action immediately after any swipe, breaking the "swipe back to cancel" affordance). Light haptic on will-open. The `trackBar` (under the OVER/UNDER pill) was thickened from 2px → 5px → 10px with `borderRadius: 5` to give a chunky, iOS-like "block" look.
