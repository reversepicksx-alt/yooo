...
- **AI Architecture**:
    ...
    - **Prediction flow**: (1) Bayesian engine computes projection + P(OVER)/P(UNDER); (2) Grok web search fetches live injury/lineup intel (`fetch_web_intel`); (3) Grok synthesis call (`call_grok`) analyses all data and writes tacticalBreakdown + reasoning + sharpSummary; (4) [BAYESIAN TRUTH] override pins recommendation and confidenceScore to the Bayesian result. Grok never overrides the math — it explains it.
