"""
BallDontLie League of Legends API client.
Base URL: https://api.balldontlie.io/lol/v1
"""
import asyncio
import time
import os
...
log = logging.getLogger("lol_client")

LOL_API_BASE = "https://api.balldontlie.io/lol/v1"
LOL_API_KEY  = os.environ.get("MLB_BDL_API_KEY", "")
...
async def _get(path: str, params: dict = None, _retry: int = 0) -> dict:
    ...
    headers = {"Authorization": LOL_API_KEY}
    url = f"{LOL_API_BASE}{path}"
    ...
    async with _rate_sem:
        ...
        try:
            ...
            raise RuntimeError(f"LoL API network error: {e}")
        finally:
            _last_req_time = time.monotonic()

        if resp.status_code != 429:
            if resp.status_code >= 400:
                raise RuntimeError(f"LoL API error {resp.status_code}: {resp.text[:200]}")
            return resp.json()

        retry_after = min(int(resp.headers.get("retry-after", "5")), 15)

    if _retry >= 2:
        raise RuntimeError("LoL API rate-limit exceeded after retries")
    log.warning(f"[LOL CLIENT] 429 on {path} — waiting {retry_after}s")
...
async def _cache_get(key: str) -> Optional[dict]:
    try:
        return await db.lol_cache.find_one({"key": key}, {"_id": 0})
    except Exception:
        return None
...
async def get_player_match_logs(player_id: int, limit: int = 30) -> list:
    ...
    for r in all_stats:
        ...
        champion = r.get("champion") or r.get("champion_name") or "Unknown"
        ...
        cs            = _safe(r.get("creep_score") or r.get("cs") or r.get("total_minions_killed") or r.get("minion_kills"))
        vision_score  = _safe(r.get("vision_score"))
        damage        = _safe(r.get("total_damage_dealt_to_champions") or r.get("damage_to_champions") or r.get("champion_damage"))
        gold          = _safe(r.get("gold_earned") or r.get("gold"))
        wards_placed  = _safe(r.get("wards_placed"))
        wards_killed  = _safe(r.get("wards_killed") or r.get("enemy_wards_killed"))
        duration      = _safe(r.get("duration") or match.get("duration"))

        kda = (kills + assists) / max(deaths, 1)
        fantasy_pts = kills * 3 + assists * 2 - deaths * 1.5 + cs * 0.01 + vision_score * 0.1
