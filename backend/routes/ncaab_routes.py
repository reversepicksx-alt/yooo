...
log = logging.getLogger("ncaab_routes")
...
async def search_ncaab_players(q: str = Query("", min_length=2), limit: int = Query(15)):
    try:
        players = await ncaab_client.search_players(q, limit=limit)
        return [{"id": p.get("id"),
        ...
        raise HTTPException(status_code=502, detail=f"NCAAB player search failed: {e}")


@router.get("/teams")
async def get_ncaab_teams():
    try:
        teams = await ncaab_client.get_teams()
        return [{"id": t.get("id"), "fullName": t.get("full_name") or t.get("name"),
                 "abbreviation": t.get("abbreviation"), "conference": t.get("conference")}
                for t in teams]
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"NCAAB teams failed: {e}")


class NcaabPredictRequest(BaseModel):
    playerName:   str
    ...
    season:       Optional[int]  = CURRENT_NCAAB_SEASON
    restDays:     Optional[int]  = None


@router.post("/predict")
async def ncaab_predict(req: NcaabPredictRequest):
    ...
    if prop_type not in ncaab_engine.NCAAB_PROPS:
        raise HTTPException(status_code=400, detail=f"Unknown NCAAB prop: {prop_type}")
    if req.line <= 0:
    ...
    if player_id:
        player_data = await ncaab_client.get_player(player_id)
    if not player_data and req.playerName:
        results = await ncaab_client.search_players(req.playerName, limit=5)
        if results:
            best = results[0]
            player_id = best.get("id")
            player_data = await ncaab_client.get_player(player_id) or best
    ...
    if not player_id:
        raise HTTPException(status_code=404, detail=f"Player '{req.playerName}' not found in NCAAB database.")

    try:
        game_logs, season_avg = await asyncio.gather(
            ncaab_client.get_player_game_logs(player_id, req.season),
            ncaab_client.get_season_averages(player_id, req.season),
        )
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Failed to fetch NCAAB data: {e}")

    if not game_logs:
        raise HTTPException(status_code=404,
                            detail=f"No stats found for {req.playerName} in the {req.season} season.")

    result = ncaab_engine.compute_ncaab_projection(
