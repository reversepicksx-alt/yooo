...
Primary AI: Gemini 2.5 Flash (all AI calls via Replit Gemini Integration).
...
async def _ai_call(
    ...
    try:
        _hit = await db.ai_response_cache.find_one({"_k": _ck}, {"_id": 0, "v": 1})
...
async def _gemini_call(
    ...
    return await _ai_call(prompt, system=system, temperature=temperature,
...
async def fetch_web_intel(
    ...
    Primary: Gemini with knowledge-based web search.
    Fallback: Gemini knowledge call (no search).
    ...
    # Strategy 1: Gemini with web search
...
async def fetch_opponent_ppda(opponent: str, league: str = "", timeout: int = 20) -> float | None:
    ...
    # Strategy 1: Gemini with web search (understat.com)
    search_prompt = (
    ...
    try:
        ...
        if text:
            if "unknown" in text.lower():
                print(f"[PPDA] Gemini knowledge fallback: {opponent} unknown")
                return None
            m = _re.search(r'\b(\d{1,2}(?:\.\d{1,2})?)\b', text)
            if m:
                val = float(m.group(1))
                if 3.0 <= val <= 30.0:
                    print(f"[PPDA] Gemini knowledge fallback: {opponent} PPDA={val}")
                    return val
    except Exception as e:
...
async def _try_settle_wc_via_gemini(pick: dict) -> bool:
    ...
    Strategy:
      ...
      3. Fall back to a knowledge-only Gemini call (no live search
    ...
    # Stage 2 (knowledge fallback) removed permanently.
    # No real-time stats and returned "0" for in-progress matches,
...
async def auto_scout_loop():
    ...
    print("[AI ENGINE] Auto-scout started")
...
async def pattern_mining_loop():
    ...
    print("[AI ENGINE] Pattern mining started")
...
_grok_call        = _ai_call
_grok_search_call = _ai_search_call
